...
theta = norm(v_rho_r);
n = v_rho_r ./ theta;
uvms.Jv_hr = ((n * n')-((theta/sin(theta)) .* (skew(v_iv) * skew(v_nd) * (eye(3) - (n * n'))))) * [zeros(3, 7), -(skew(v_rp) * (eye(3)-(v_kw * v_kw'))) ./ (norm(v_rp) * norm(v_rp)), -eye(3)];

% Jacobian for the distance to rock task
uvms.Jv_dr = [zeros(3, 7) -(eye(3)-(v_kw * v_kw')) zeros(3)];
