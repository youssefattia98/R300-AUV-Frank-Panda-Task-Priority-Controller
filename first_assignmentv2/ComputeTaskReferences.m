...
uvms.xdot.v_dr = 0.2 * -((v_rp(1:2) - uvms.vTb(1:2, 4)) ./ norm(v_rp(1:2) - uvms.vTb(1:2, 4))) * (norm(v_rp(1:2) - uvms.vTb(1:2, 4)) - 0.5);
uvms.v_rp = v_rp(1:2);
...
for i = 1:size(uvms.jlmin)
    ...
    if uvms.q(i) <= mean(i)
        uvms.xdot.jl(i) = 0.2 * (uvms.jlmin(i) - uvms.q(i));
    else
        uvms.xdot.jl(i) = 0.2 * (uvms.jlmax(i) - uvms.q(i));
    end
end
