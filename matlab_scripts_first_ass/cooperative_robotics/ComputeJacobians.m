...
% Jacobian for the heading task
uvms.Jv_hr = uvms.Jv_o;

% Jacobian for the fix-base translational task
uvms.Jv_fbt = [zeros(3, 7) eye(3) zeros(3)];

% Jacobian for the fix-base rotational task
uvms.Jv_fbr = [zeros(3, 7) zeros(3) eye(3)];
...
% Jacobian for the upper joint limit task:
uvms.Jv_ujl = [eye(7) zeros(7, 6)];

% Jacobian for the lower joint limit task:
uvms.Jv_ljl = [eye(7) zeros(7, 6)];
